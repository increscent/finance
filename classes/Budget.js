...
class Budget {
  constructor(account, budgets, transactions, budget_name) {
    this.account = account;
    this.budgets = budgets;
    this.budget = this.budgets.find(x => x.name == budget_name);
  }

  save(new_budget, callback) {
    if (this.budget) {
      // budget exists and will be updated
      this.handleChangedBudgetName(this.budget, new_budget, (error, new_budget_1) => {
        console.log(error);
        if (error) return callback(error);
        console.log(new_budget_1);
        this.handleChangedBudgetAllowance(new_budget_1, new_budget, (error, new_budget_2) => {
          console.log(error);
          if (error) return callback(error);
          console.log(new_budget_2);
          return callback(null, new_budget_2)
        });
      });
    } else {
      // budget must be created
      if (!this.isValidName(new_budget.name)) return callback('Invalid Name');
      var budget = new Models.Budget(new_budget);
      budget.save()
      .then(budget => {
