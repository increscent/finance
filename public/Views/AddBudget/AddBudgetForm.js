...
class AddBudgetForm extends mixin(Form, React.Component) {
  ...
  handleBudgetDelete(e) {
    e.preventDefault();
    Store.deleteBudget(this.props.uri)
    .then(() => {
  ...
  handleFormSubmit(e) {
    ...
    var rules = [
      ...
      {name: "name", validate: (x) => Store.budgets.find((budget) => x.trim().toLowerCase() != budget.name.trim().toLowerCase()), error_message: "That name already exists. Please enter a unique budget name."},
